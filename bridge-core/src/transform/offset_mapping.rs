use crate::transform::{
    ConsumerGroup, Offset, Partition, Topic,
...
pub fn insert_offset_transformations(
    ...
    match transformations.get_mut(&consumer_group) {
        Some(transformations_for_consumer_group) => {
            ...
            transformations_for_consumer_group.push((topic, partition, *source_offset_from_message, *current_offset_from_message));
        }
        None => {
            let mut offsets: HashMap<i64, i64> = HashMap::new();

            offsets.insert(*source_offset_from_message, *current_offset_from_message);
            transformations.insert(consumer_group, vec![(topic, partition, *source_offset_from_message, *current_offset_from_message)]);
            info!(
